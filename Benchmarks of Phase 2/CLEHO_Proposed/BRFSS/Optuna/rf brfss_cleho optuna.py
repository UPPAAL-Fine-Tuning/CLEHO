...
def objective_rf_augmented(trial):
    ...
    score = cross_val_score(model, X_train_aug, y_train_aug, cv=5, scoring="accuracy").mean()
    return score


print(f" Starting Optuna Optimization (Categorical Search) on {X_train_aug.shape[1]} features...")
study_rf = optuna.create_study(direction="maximize")
start_time = time.time()

study_rf.optimize(objective_rf_augmented, n_trials=15, show_progress_bar=True)
...
print("\n" + "█"*60)
print(" PERFORMANCE REPORT: RF ON AUGMENTED BRFSS DATA")
print("█"*60)
